...
PP_VOID vTaskAudio(PP_VOID *pvData)
{
    ...
    for(;;)
    {
        ...
        if(gHandle[myHandleNum].phEventGroup)
        {
            ...
            {
                // process event bit
                if(eventResultBits & (1<<EVENT_AUDIO_INIT))
                {/*{{{*/
                    //Called once when start task.
                    //No message Queue.
                    //printf("%s:Rcv Event[%dbit(%s)]\n", psTaskName[myHandleNum], EVENT_AUDIO_INIT, psEventNameAudio[EVENT_AUDIO_INIT]);
                }/*}}}*/
                if(eventResultBits & (1<<EVENT_AUDIO_MSG))
                {/*{{{*/
                    LOG_DEBUG("%s:Rcv Event[%dbit(%s)]\n", psTaskName[myHandleNum], EVENT_AUDIO_MSG, psEventNameAudio[EVENT_AUDIO_MSG]);
                    {/*{{{*/
                                ...
                                for(i = 0; i < queueCnt; i++)
                                {
                                    if( OSAL_QUEUE_CMD_Receive(gHandle[myHandleNum].phQueue, &queueItem, 0) == eSUCCESS )
                                    {
                                        /*
                                           ...
                                           */
                                        if(queueItem.u32Cmd == CMD_AUDIO_ALIVE)
                                        {						
                                        }
                                        ...
                                        if(queueItem.u16Attr)
                                        {
                                            if(queueItem.u16Attr & (1<<QUEUE_CMDATTR_BIT_REQACK)) 
                                            {
                                                if(queueItem.u32Cmd == CMD_AUDIO_ALIVE)
                                                {						
                                                    AppTask_SendCmd(CMD_ACK, TASK_AUDIO, queueItem.u16Sender, 0, &pstParam->u32TaskLoopCnt, sizeof(pstParam->u32TaskLoopCnt), 1000);
                                                }
                                                else
                                                {
                                                    AppTask_SendCmd(CMD_ACK, TASK_AUDIO, queueItem.u16Sender, 0, NULL, 0, 1000);
                                                }
                                            }
                                        }
