...
/* For Kernel debugging. Declare vAssertCalled() on "port.c"  */
/*
 * Debugging Options
 */
#undef CONFIG_DEBUG
...
#if 1
extern void AssertCalled(const char *pcFile, const char *pcFunc, unsigned int uLine);

#define configASSERT(x) {if((x)==0) AssertCalled(__FILE__,__func__,__LINE__);}
#endif
